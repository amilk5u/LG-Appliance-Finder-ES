...
let Plumbed = 'FV65356197';
let Non_Plumbed = 'FV65356198';
let Water_Only = 'FV65356195';

let Pure_N_Fresh = 'FTV0338323V';
...
let ThinQ = 'FTV0335143V';

// color
let black = 'FTV0335778V';
let stainless_steel = 'FTV0339412V';
let silver = 'FTV0335780V';
let white = 'FV65357088';
let beige = 'FTV0335777V';

let Dummy = 'dummy'; // dummy (더미데이터) 어느제품이나 상관없음 옵션
let Dummy2 = 'step05_have_nothing'; // 어느제품이나 상관 없음 옵션
let AllSelectOption = 'AllSelectOption'; // All Select Option
 
// fautre Key
let Key_Instaview = 'FT06515762';
let Key_Pure_N_Fresh = 'FT06516794';
/* 변수 정의 end -------------------------------*/
...
const product = {
   ...
   'GMX945MC9F': { // 1
      ...
      FT06515502: ThinQ,
      FT06516792: [Plumbed, Pure_N_Fresh, Instaview],
      FT06515694: black
   },
   'GML945NS9E': { // 2
      FT05707162: multi,
      ...
      FT06515502: ThinQ,
      FT06516792: [Plumbed, Pure_N_Fresh],
      FT06515694: silver
   },
   'GMX844MC6F': { // 3
      FT05707162: multi,
   ...
   'GMQ844MC5E': { // 4
      ...
      FT06515693: EnergyGrade_E,
      FT06515502: ThinQ,
      FT06516792: [Pure_N_Fresh, Instaview],
      FT06515694: black
   },
   'GML844PZ6F': { // 5
      FT05707162: multi,
   ...
   'GSXV91BSAF': { // 1
      ...
      FT06515693: EnergyGrade_E,
      FT06515502: ThinQ,
      FT06516792: [Plumbed, Non_Plumbed, Instaview],
      FT06515694: silver
   },
   'GSXV91MCAE': { // 2
      FT05707162: american,
   ...
   'GSLV51WBXM': { // 12
      ...
      FT06515693: EnergyGrade_F,
      FT06515502: '',
      FT06516792: [Plumbed, Non_Plumbed],
      FT06515694: black
   },
   'GSJV31DSXF': { // 13
      FT05707162: american,
   ...
   object: [
      ...
      },
      {
         ...
         screenImg: {
            ...
            lastScreenImg: 'step06/american_que_img04.png',
      ...
      },
      {
         ...
         screenImg: {
            ...
            lastScreenImg: 'step06/tall_que_img04.png',
      ...
      },
      {
         ...
         screenImg: {
            ...
            lastScreenImg: 'step06/double_que_img04.png',
      ...
      },
      {
         ...
         screenImg: {
            ...
            lastScreenImg: 'step06/lader_que_img04.png',
      ...
      [
         {
            ...
            changeData: {
               description: '',
               screenImg: 'step05/que_img01.png',
            }
         },
         {
            key: 'FT06515693',
...
function main() {
   ...
   $backBtn.on('click', function () {
      let _judgmentStep = 'backStep'; // 이전 스텝
      if (idx > 0) {
         idx--;
         stepUpdateEvent(idx, _judgmentStep);
      }
   ...
   function stepUpdateEvent(idx, _judgmentStep) {
      ...
      if (idx === 2) {
         ...
         for (let i = 0; i < _currentHtml.length; i++) {
            ...
            if (_currentHtml[i].value === Dummy || _currentHtml[i].value === Dummy2) {
               $selectWrap.find('ol li').eq(i).find('.answer_btn').addClass('not_class');
            }
            /* 상관없음 옵션을 선택 했을 때 230207 start */
         }
      }
      ...
      if (idx > 1 && matchingProducts[idx - 1] === undefined) { // back 했을때를 고려해서 undefined 구분
         if (idx === 3) {
            ...
            for (let i = 0; i < _lastPro.length; i++) {
               ...
               // console.debug('선택된 제품 카운트 : ', _valueCounting, '선택된 제품 : ', _lastPro[i])
               for (let j = 0; j < stepCount[stepCount.length - 1]; j++) {
                  let _selectKey = selectedParameters[selectedParameters.length - (1 + j)].split('=')[0]; // key
                  let _selectValue = selectedParameters[selectedParameters.length - (1 + j)].split('=')[1]; // value
                  let _dataTrueBoolean = false;

                  if (_selectValue === Dummy || _selectValue === Dummy2) { // 더미가 선택 됬을 경우
               ...
               // console.log('All Select유무 : ', _AllSelectBol, '카운트 갯수 :  : ', _valueCounting, 'true 된 갯수 :  : ', _judgmentNum)
               if (_AllSelectBol) { // All Select 가 있으면 - 1
                  _valueCounting--;
               }
               // 선택한 벨류값의 갯수와 true 된 갯수와 같으면 제품 추출
               if (idx === 5 && _judgmentNum === _valueCounting) {
      ...
      if (idx !== 0) {
         ...
         $('.answer_btn').each(function () {
            ...
            if (_this.hasClass('not_class')) {
               _this.removeAttr('disabled');
            }
         });

         /* 상관없음 옵션 밖에 노출 안될 시 전체선택 비활성화 230207 start */
         let openOptionNum = 0;
         // 매칭되지 않아서 옵션이 오픈 되지 않았을 때 팝업 생성
         $('.answer_btn').each(function () {
            if ($(this).data('value') !== AllSelectOption && $(this).attr('disabled') === undefined && !$(this).hasClass('not_class')) {
               openOptionNum++
            }
         });
     
         // 매칭된 제품이 없을 때
         if (_lastPro.length === 0) {
            $('#quickFinder').addClass('not_matched');
            $('.all_select').prop('disabled', true);
            $(".not_class").prop('disabled', true); 
         } else {
         ...
         /* 상관없음 옵션 밖에 노출 안될 시 전체선택 비활성화 230207 end */
      ...
      if (_judgmentStep === 'backStep') {
         ...
         //    idx === 1 ? _lastPro = matchingProducts[matchingProducts.length - 1] : _lastPro = productStorage; // 매칭 된 제품
   ...
   function answerSelectEvent(idx, _htmlIdx) {
      ...
      $('.answer_btn').on('click', function () {
         ...
         /* 상관없음 옵션 있을 시 활성화 버튼 -1 230207 end */
               ...
               if (!_this.hasClass('active')) {
                  ...
                  $('.answer_btn').each(function () {
                     ...
                     if (!$(this).hasClass('active') && $(this).attr('disabled') === undefined && $(this).data('value') !== AllSelectOption) { // acitve 없고, diabled 없고, All Select 가 아닌 버튼의 kay / value 값                         
                        if (!$(this).hasClass('not_class')) { // 상관없음 옵션이 있을 때 제외하고 push
                           selectedParameters.push($(this).data('key') + '=' + $(this).data('value')); // push
                        }
                     }
                     if ($(this).attr('disabled') === undefined && !$(this).hasClass('not_class')) {
                        $(this).addClass('active');
                     }
                  });
                  $('.not_class').removeClass('active');
                  selectedParameters = selectedParameters.filter(item => {
                     return item !== $('.not_class').data('key') + '=' + $('.not_class').data('value');
                  });
            ...
            if (idx === 0) {
               ...
               if (!_this.hasClass('active')) {
                  ...
                  if (_this.hasClass('not_class')) {
                     // 모두 해제
                     $('.answer_btn').each(function () {
                        let _currentKeyValue = $(this).data('key') + '=' + $(this).data('value'); // 현재 선택한 키/벨류 ex) Q2=Q2_value2
                        if ($(this).attr('disabled') === undefined && $(this).data('value') !== AllSelectOption && !$(this).hasClass('not_class')) { // acitve 없고, diabled 없고, All Select 가 아닌 버튼의 kay / value 값 
                           $(this).hasClass('not_class');
                           $(this).removeClass('active');
                           selectedParameters = selectedParameters.filter(item => {
                              return item !== _currentKeyValue;
                           });
                        }
                        if ($(this).data('value') === AllSelectOption) {
                           $(this).removeClass('active');
                           selectedParameters = selectedParameters.filter(item => {
                              return item !== _AllSelectKeyValue;
                           });
                        }
                     });
                  } else {
                     $('.not_class').removeClass('active');
                     let _currentKeyValue = $('.not_class').data('key') + '=' + $('.not_class').data('value');
                     selectedParameters = selectedParameters.filter(item => {
                        return item !== _currentKeyValue;
                     });
                  }
                  /* 상관없음 옵션을 선택 했을 때 230207 end */
                  ...
                  if (idx !== 2) {  //step03 제외
                     ...
                     // All Select Option Active 삭제 
                     selectedParameters = selectedParameters.filter(item => {
                        return item !== _AllSelectKeyStep3Value;
                     });
                  }
               }
               if (idx !== 2) { // step03 제외
                  ...
                  $('.answer_btn').each(function () {
                     if ($(this).attr('disabled') === undefined && $(this).data('value') !== AllSelectOption) {
                        if ($(this).hasClass('active') && !$(this).hasClass('not_class')) {
                           _activeBtn++;
                        }
                     }
                  });
                  ...
                  // console.log(_activeBtn === _btnAllCount, _activeBtn, _btnAllCount)
         ...
         //          productStorage = _lastPro
         //       }
         //       console.log('productStorage : ', productStorage);
         //    } else {
         //       let _lastPro = matchingProducts[matchingProducts.length - 1]; // 라스트 추출 제품 가져오기

         //       if (_this.data('value') !== Dummy) {
         //          productStorage = []; // 제품 데이터 초기화 
         //       }
         ...
         //          }
         //          // 선택한 벨류값의 갯수와 true 된 갯수와 같으면 제품 추출
         //          if ((idx === 4) && _judgmentNum === _valueCounting) {
         //             // console.debug('true 된 갯수 : ', _judgmentNum, '카운트 갯수 : ', _valueCounting, _judgmentNum === _valueCounting)
         //             _stepProductArray.push(_lastPro[i]);
         //          }
         //       }
         ...
         // if (idx !== 0) {
         //    var _dataValue = []; // 추출된 제품에서 현재스텝의 마크업과 비교될 value 값 추출 배열
         //    let _lastPro = _stepProductArray;
         //    let _currentKey = []; // 현재 스텝의 key 값
         //    let _currentKeyRemoval = []; // 중복된데이터 제거된 key 값
      ...
      for (let i = 0; i < selectedParameters.length; i++) {
         ...
         if (_val !== AllSelectOption) {
            ...
            if (selectedParameters[i].includes(',')) {
               ...
               /* feature key 예외 처리 start */
               if (_val === Instaview) {
                  result += Key_Instaview + '=' + _val + '&'
               } else if (_val === Pure_N_Fresh) {
                  result += Key_Pure_N_Fresh + '=' + _val + '&'
                  /* feature key 예외 처리 end */
               } else if (_val === Dummy || _val === Dummy2) {
                  result += ''
               } else {
